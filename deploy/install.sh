...
INSTALL_DIR="/opt/viz2ai"
SERVICE_NAME="viz2ai"
SERVICE_USER="viz2ai"
CONFIG_DIR="/etc/viz2ai"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="viz2ai 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 viz2ai 服务："
    ...
    ["not_installed"]="viz2ai 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 viz2ai。"
    ...
    ["uninstall_complete"]="viz2ai 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 viz2ai"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 viz2ai"
    ...
    ["server_config_desc"]="配置 viz2ai 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="viz2ai Installation Script"
    ...
    # Completion
    ["install_complete"]="viz2ai installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start viz2ai service:"
    ...
    ["not_installed"]="viz2ai is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove viz2ai from your system."
    ...
    ["uninstall_complete"]="viz2ai has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install viz2ai"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove viz2ai"
    ...
    ["server_config_desc"]="Configure viz2ai server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/viz2ai" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/viz2ai" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="viz2ai_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/viz2ai" "$INSTALL_DIR/viz2ai"
    chmod +x "$INSTALL_DIR/viz2ai"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/viz2ai"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/viz2ai.service << EOF
[Unit]
Description=viz2ai - AI API Gateway Platform
...
User=viz2ai
Group=viz2ai
WorkingDirectory=/opt/viz2ai
ExecStart=/opt/viz2ai/viz2ai
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=viz2ai
...
start_service() {
    ...
    if systemctl start viz2ai; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u viz2ai -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable viz2ai 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status viz2ai"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u viz2ai -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart viz2ai"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop viz2ai"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/viz2ai" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet viz2ai; then
        print_info "$(msg 'stopping_service')"
        systemctl stop viz2ai
    fi

    # Backup current binary
    cp "$INSTALL_DIR/viz2ai" "$INSTALL_DIR/viz2ai.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/viz2ai.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/viz2ai"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start viz2ai

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: viz2ai must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/viz2ai" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="viz2ai.backup.${current_version}"
        else
            backup_name="viz2ai.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/viz2ai" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop viz2ai 2>/dev/null || true
    systemctl disable viz2ai 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/viz2ai.service
    systemctl daemon-reload
