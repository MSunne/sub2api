...
# 或：
#   sudo ./install-datamanagementd.sh --source /path/to/viz2ai/repo

BIN_PATH=""
SOURCE_PATH=""
INSTALL_DIR="/opt/viz2ai"
DATA_DIR="/var/lib/viz2ai/datamanagement"
SERVICE_FILE_NAME="viz2ai-datamanagementd.service"
...
参数:
  ...
  --source  指定 viz2ai 仓库路径（脚本会执行 go build）
  -h, --help 显示帮助

示例:
  sudo ./install-datamanagementd.sh --binary ./datamanagement/datamanagementd
  sudo ./install-datamanagementd.sh --source /opt/viz2ai-src
EOF
}
...
if ! id viz2ai >/dev/null 2>&1; then
  echo "[2/6] 创建系统用户 viz2ai..."
  useradd --system --no-create-home --shell /usr/sbin/nologin viz2ai
else
  echo "[2/6] 系统用户 viz2ai 已存在，跳过创建"
fi
...
echo "[4/6] 准备数据目录..."
mkdir -p "$DATA_DIR"
chown -R viz2ai:viz2ai /var/lib/viz2ai
chmod 0750 "$DATA_DIR"
...
systemctl enable --now viz2ai-datamanagementd

echo "[6/6] 完成，当前状态："
systemctl --no-pager --full status viz2ai-datamanagementd || true

cat <<'EOF'

下一步建议：
1. 查看日志：sudo journalctl -u viz2ai-datamanagementd -f
2. 在 viz2ai（容器部署时）挂载 socket:
