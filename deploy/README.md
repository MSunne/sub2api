# viz2ai Deployment Files

This directory contains files for deploying viz2ai on Linux servers.
...
| `viz2ai.service` | Systemd service unit file |
| `viz2ai-datamanagementd.service` | datamanagementd systemd service unit file |
...
# View logs
docker compose -f docker-compose.local.yml logs -f viz2ai

# If admin password was auto-generated, find it in logs:
docker compose -f docker-compose.local.yml logs viz2ai | grep "admin password"
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs viz2ai | grep "admin password"
   ```
...
- 主进程固定探测 `/tmp/viz2ai-datamanagement.sock`
...
# Restart viz2ai only
docker compose -f docker-compose.local.yml restart viz2ai
...
# View logs
docker compose logs -f viz2ai

# Restart viz2ai only
docker compose restart viz2ai
...
| `ADMIN_EMAIL` | No | `admin@viz2ai.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf viz2ai-complete.tar.gz deployment/

# Transfer to new server
scp viz2ai-complete.tar.gz user@new-server:/path/to/destination/

# On new server: Extract and start
tar xzf viz2ai-complete.tar.gz
cd deployment/
docker compose -f docker-compose.local.yml up -d
```
...
viz2ai supports three methods to connect to Gemini:
...
2. Extract and copy the binary to `/opt/viz2ai/`
3. Copy `viz2ai.service` to `/etc/systemd/system/`
4. Run:
   ```bash
   sudo systemctl daemon-reload
   sudo systemctl enable viz2ai
   sudo systemctl start viz2ai
   ```
...
# Stop the service
sudo systemctl stop viz2ai

# Restart the service
sudo systemctl restart viz2ai

# Check status
sudo systemctl status viz2ai

# View logs
sudo journalctl -u viz2ai -f
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/viz2ai.service
   ```
...
The main config file is at `/etc/viz2ai/config.yaml` (created by Setup Wizard).
...
/opt/viz2ai/
├── viz2ai              # Main binary
├── viz2ai.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 viz2ai
...
# View detailed logs
docker compose logs --tail=100 viz2ai
...
# View recent logs
sudo journalctl -u viz2ai -n 50

# Check config file
sudo cat /etc/viz2ai/config.yaml
...
viz2ai supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).
