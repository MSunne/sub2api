  ...
  <BaseDialog :show="showAuditModal" title="请求审计详情" width="wide" @close="closeAuditModal">
    <div class="space-y-4">
      ...
      <template v-else-if="selectedAudit">
        ...
        <RequestAuditBodyPanel v-show="auditBodyTab === 'request'" :audit="selectedAudit" role="request" :load-body="loadUserAuditBody" />
        <RequestAuditBodyPanel v-show="auditBodyTab === 'response'" :audit="selectedAudit" role="response" :load-body="loadUserAuditBody" />
      </template>
    </div>
  </BaseDialog>
